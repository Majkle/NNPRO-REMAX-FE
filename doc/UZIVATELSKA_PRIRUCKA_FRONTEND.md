...
### 11.1 Pravidelné úkoly

#### Týdenní
- Kontrola outdated dependencies: `npm outdated`
- Spuštění testů: `npm run test:coverage`
- Code review pull requests

#### Měsíční
- Aktualizace minor/patch verzí: `npm update`
- Security audit: `npm audit`
- Bundle size check: `npx source-map-explorer 'build/static/js/*.js'`

#### Kvartálně
- Aktualizace major verzí (opatrně!)
...
# Oprava včetně breaking changes (opatrně!)
npm audit fix --force
```

### 11.3 Monitoring v produkci
...
### Q: Jak změním port dev serveru?
A: Nastavte environment proměnnou `PORT`:
```bash
# Windows
...
# Nebo v .env.development
PORT=3001
```

### Q: Jak vypnu automatic browser opening?
A: V `.env.development`:
```env
BROWSER=none
```

### Q: Jak používám proxy pro API v development?
A: V `package.json`:
```json
{
...
```
Pak můžete volat `/api/...` místo `http://localhost:8080/api/...`

### Q: Jak přidám custom environment proměnnou?
A: Všechny proměnné musí začínat `REACT_APP_`:
```env
REACT_APP_CUSTOM_VAR=hodnota
```
Použití:
```typescript
const value = process.env.REACT_APP_CUSTOM_VAR;
```

### Q: Jak debuguji production build lokálně?
A:
```bash
npm run build
npx serve -s build
...
### Q: Proč se změny v .env neprojevují?
A: Je potřeba restartovat dev server (Ctrl+C, pak `npm start`)

## 13. Další zdroje

### 13.1 Oficiální dokumentace
...
- React dokumentace (česky): https://cs.react.dev/
...
- Development team: dev@remax.cz
- Project manager: pm@remax.cz
